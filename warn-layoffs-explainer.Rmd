...
This walkthrough gathers the data to help answer those questions via a scraping exercise in R. We'll go from a clunky government website to a single data table that shows the business impact, still ongoing, of coronavirus over time.

### WARN listings

Companies that have at least 100 employees are required by law to submit notices to the state under the Worker Adjustment and Retraining (WARN) Act. There are other specifics under the law, but the 100-employee rule broadly holds - and some companies with less than 100 employees issue notices, too. WARN does not apply to government agencies.

Here's how the state organizes listings online:

![*The WARN homepage in Arizona*](WARNhomepage.png)

The website makes it difficult to get data. Every row links to a new page with details about how many people were laid off at the company and there's no bulk download button. To collect data on a large scale, we'd have to undertake a lot of time-consuming, error-prone clicking around. 

![*If I click on Yelp's company name, I see the detailed notice - 375 layoffs.*](yelp.png)
...
When I page through the site, I see that a maximum of 25 layoffs can be displayed at one time. Typing over the 25 with a larger number doesn't change that. The website appears to be one giant table with many rows: Page 2 shows rows 26-50, for example, and page 3 shows 51-75. 
...
I'll do a test run on the first page of results, saving the first page of results. I set the `start_row` equal to `1`, and changed the `orderby` header to sort descending by date:
...
As we begin looping, we need only to change `start_row` - everything else in the URL stays the same. The code below shows how that works, saving each page to a folder called `warn_listing`:
...
We can scrap the first two rows and start looking for data in the third, one column at a time.
```{r}
...
We can automate the data parsing now that we understand the data structure on every page.
...
Half our work is done. Now let's prep for the next step: scraping the individual WARN URLs.
...
On this task, we don't need to do any manual parsing - so html_table will be a big help. All I'll do is pivot the data once it's parsed, and add an id column.
...
Just as before, I'll create a table to hold the data:
```{r}
...
I'll save the results as a csv.
...
Let's compile the two sets of data into one table about layoffs. I'll call it `warn_data_complete`:
```{r}
...
Below is a list of monthly layoffs, ranked by most employees impacted. March 2020 tops the list by far; April is second on the list, though it's only halfway through. Putting the two months together, we can estimate that at 9,000 people have been impacted by layoffs or furloughs in Arizona since covid-19 has spread. It's a sobering record — and one that still leaves out entrepreneurs, small businesses and government employees on furlough.
